...
export default function Sidebar({ collapsed, setCollapsed }: SidebarProps) {
  ...
  const getRoleColor = (role: string) => {
    switch (role) {
      ...
      case 'senior': return 'text-teal-700 bg-teal-500/5 border-teal-500/20';
      case 'junior': return 'text-emerald-700 bg-emerald-500/5 border-emerald-500/20';
  ...
  return (
    <aside
      className={`fixed inset-y-0 left-0 z-20 flex flex-col border-r border-border bg-card text-foreground transition-all duration-300 ${collapsed ? 'w-20' : 'w-64'
        }`}
    >
      {/* Brand Header */}
      <div className="flex h-16 items-center justify-between px-6 border-b border-border">
        <Link href="/dashboard" className="flex items-center gap-2 font-black text-foreground">
          <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-primary to-teal-600 shadow-md">
        ...
        {filteredNavItems.map((item) => {
          ...
          return (
            <Link
              key={item.name}
              href={item.href}
              className={`group flex items-center gap-3 rounded-xl px-3 py-3 text-xs font-bold uppercase tracking-wider transition-all duration-150 cursor-pointer ${isActive
