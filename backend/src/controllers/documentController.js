...
export const createDocVersion = async (req, res, next) => {
  try {
    const { id, document_id, revision_number, file_url, file_size, change_summary, created_by, built_up_area } = req.body;
    ...
    const { data, error } = await supabase
      .from('doc_versions')
      .insert({ id, document_id, revision_number, file_url, file_size, change_summary, created_by, built_up_area })
      .select()
      .single();
    if (error) throw new Error(error.message || JSON.stringify(error));
...
export const uploadDrawing = async (req, res, next) => {
  try {
    const { document_id, filename, base64Data, changelog, created_by, version_id, built_up_area } = req.body;
    ...
    if (version_id) {
      ...
      const { data: updatedVersion, error: updateErr } = await supabase
        ...
        .update({
          ...
          built_up_area: built_up_area ? Number(built_up_area) : undefined
        })
        .eq('id', version_id)
        .select()
        .single();
      ...
      const { data: newVersion, error: insertErr } = await supabase
        ...
        .insert({
          ...
          created_by,
          built_up_area: built_up_area ? Number(built_up_area) : 0,
