  ...
  return (
    <div className="space-y-8 select-none font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-6 border-b border-slate-200">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight text-slate-900 font-mono uppercase">Studio Teams & Workloads</h1>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      {u.avatar_url ? (
                        ...
                        <div className="h-8 w-8 rounded-full bg-slate-800 flex items-center justify-center text-xs font-bold">{u.name[0]}</div>
                      )}
                      <div>
                    ...
                    <div className="w-full h-1.5 bg-secondary rounded-full overflow-hidden">
                      <div 
                        className={`h-full rounded-full ${
                          rating === 'Overloaded' ? 'bg-rose-500' : rating === 'Underloaded' ? 'bg-slate-400' : 'bg-emerald-500'
                        }`}
                ...
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    {proj ? (
                      ...
                      <span className="text-xs text-slate-500 block mt-1">Unassigned project</span>
                    )}
                  </div>

                  <span className="text-[10px] font-mono font-bold text-slate-500 bg-secondary px-2.5 py-1 rounded-xl border border-border">
      ...
      {showCreateTeamModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={() => setShowCreateTeamModal(false)} />
